...
- **2026-07-12** — Implemented (CU-1..CU-8) and reconciled (no logic-level decision changed): while executing CU-2/CU-4 the plan-header→tech.md pointers were made ownership-correct — `Alternatives considered` points to the logic spec (rationale is logic-owned), and a `Cross-cutting` section was added to the tech.md template so that pointer resolves. Spec-internal rule-IDs (R1/R2/R4/R5) were de-labelled in the shipped skills so they stay self-contained. Reflected in the `writing-plans` and `writing-tech-spec` skills.
