...
- **C2.9 After authoring:** dispatch the CU-3 reviewer; apply fixes inline; offer an **optional** user skim (not a gate) — and with that offer, **surface the tech spec's most consequential calls**: which subsystem/files it anchored in, its major DO-NOT-CHANGE fences, and its riskiest contracts, so the user can veto a legal-but-wrong technical bet without reading the whole doc (this is the F9 hedge — it targets the wrong-but-*consistent* choice the automated reviewer, which only checks fidelity/precision, cannot catch). Then hand to `writing-plans` for planning. Implements [logic §Key decisions → review gates](./logic.md#key-decisions--rationale).
